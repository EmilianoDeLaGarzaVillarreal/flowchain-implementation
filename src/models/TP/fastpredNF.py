...
class fastpredNF_TP(nn.Module):
    ...
    def predict_from_new_obs(self, data_dict: Dict, time_step: int) -> Dict:
        ...
        import pdb
        # pdb.set_trace()
        base_log_prob = self.flow.base_dist( #CAN CHANGE unsqueeze
            (data_dict["gt_st"][:, time_step-1]).unsqueeze(1), step=time_step).log_prob(data_dict[("prob_st", 0)][:, :, time_step-1, :2])
        base_log_prob = torch.sum(base_log_prob, dim=-1)
        base_prob = base_log_prob.exp() #CAN CHANGE keepdim
        base_log_prob = (base_prob / base_prob.sum(dim=1, keepdim=True) *
