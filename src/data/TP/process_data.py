        ...
        for subdir, dirs, files in os.walk(
            ...
            for file in files:
                if file.endswith(".txt"):
                    ...
                    for node_id in unique(data["node_id"]):
...
for desired_source in ["simline", "simfork", "simcross"]:
    for data_class in ["train", "val", "test"]:
        ...
        traj_base = []
        for sgn in [1, -1]:
            traj_flat_x = np.arange(-obs_len - 1, 0.0, 1.0, dtype=float)
            traj_flat_y = np.zeros_like(traj_flat_x, dtype=float)
            traj_flat = np.concatenate(
                [traj_flat_x[:, None], traj_flat_y[:, None]], axis=1
            )

            if desired_source == "simline":
                dist = np.arange(1.0, pred_len + 1.0, 1.0, dtype=float)
                traj_ramp_x = dist
                ...
                dist = np.concatenate(
                    [
                        np.arange(1.0, pred_len // 3 + 1.0, 1.0, dtype=float),
                        np.arange(
                            pred_len // 3 - 1.0, -pred_len // 3 - 1.0, -1.0, dtype=float
                        ),
                    ]
                )
                traj_ramp_y = dist * np.sin(sgn * theta, dtype=float)

            traj_ramp = np.concatenate(
                [traj_ramp_x[:, None], traj_ramp_y[:, None]], axis=1
            )
        ...
        traj += np.random.normal(
            loc=np.zeros_like(traj), scale=np.ones_like(traj) * std
        )
        ...
        for node_id in pd.unique(data["node_id"]):
            ...
            node_data = pd.DataFrame(data_dict, columns=data_columns)
            node = Node(
                node_type=env.NodeType.PEDESTRIAN, node_id=node_id, data=node_data
            )
            node.first_timestep = new_first_idx
