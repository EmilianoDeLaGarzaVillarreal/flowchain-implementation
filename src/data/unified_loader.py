...
def unified_loader(cfg: CfgNode, rand=True, split="train", batch_size=None,
                   partition_id: int = 0, num_partitions: int = 1) -> DataLoader:
    # train, val, test
    if cfg.DATA.TASK == "TP":
    ...
    if partition_id > -1:
        ...
        end_idx = (partition_id + 1) * partition_size if partition_id < num_partitions - 1 else total_len
