...
from task import federatedTrain, federatedTest, test
import flwr as fl
...
class FlowerClient(NumPyClient):
    ...
    def fit(self, parameters, config):
        print(
            f"[Client {
                self.partition_id
            }] fit() called — loading parameters, about to train..."
        )
        self.set_parameters(parameters)
        print(
            f"[Client {self.partition_id}]  train loader size: {
        ...
        print(
            f"[Client {self.partition_id}] fit() returning — trained for {
                self.local_epochs
            } local epochs"
        )
        return (
