from custom_strategy.FedProxCustom import FedProxCustom
import flwr as fl
import argparse
import numpy as np
...
class LoggingFedProx(FedProxCustom):
    def configure_fit(self, server_round, parameters, client_manager):
        config = {"current_round" : server_round, "mu" : self.proximal_mu}
    ...
    def aggregate_evaluate(self, server_round, results, failures):
        if not results:
            print(f"[Server - FedProx] Round {server_round} - No evaluation results received.")
            return None, {}
...
def main():
    ...
    parser.add_argument(
        "--mu", type=float, default=0.1, help="Proximal MU"
    )
