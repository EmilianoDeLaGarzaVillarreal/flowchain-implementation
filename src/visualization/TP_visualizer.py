...
from scipy.interpolate import griddata, RBFInterpolator, interp2d, RectBivariateSpline
                ...
                for k in list(data_dict.keys()):
                    if k[0] == "prob":
                        ...
                        if ("gt_traj_log_prob", k[1]) not in data_dict:
                            ...
                            # gt_traj_prob = np.array([interp2d(xx, yy, zz_batch[:, :, t].cpu().numpy())(gt[:, t, 0].cpu().numpy(), gt[:, t, 1].cpu().numpy()) for t in range(timesteps)])
                            grid_y = yy[:, 0]  # shape: (num_y,)
                            grid_x = xx[0, :]  # shape: (num_x,)

                            gt_traj_prob = np.array([
                                RectBivariateSpline(grid_y, grid_x, zz_batch[0, :, t].reshape(len(grid_y), len(grid_x)))
                                .ev(gt[:, t, 1], gt[:, t, 0])
                            ...
                            if torch.sum(torch.isnan(gt_traj_log_prob) + torch.isinf(gt_traj_log_prob)) > 0:
                                ...
                                valid = gt_traj_log_prob[~mask]

                                if valid.numel() > 0:
                                    value = valid.min()
                                else:
                                    # fallback when everything is NaN/Inf
                                    value = torch.tensor(0.0, device=gt_traj_log_prob.device)
